...
@pytest.mark.parametrize(
    ...
    [
        (
            "local slots: table={} local hits: integer=0 "
            "for i=1,60 do local k: integer=i*3 slots[k]=true "
            "if slots[k] then hits=hits+1 end end return hits",
            60,
        ),
        (
            "local visited: table={} local first: integer=0 "
            "for i=1,50 do local k: integer=i*5 if not visited[k] then "
            "visited[k]=true first=first+1 end end return first",
            50,
        ),
        (
            "local occupied: table={} local total: integer=0 "
            "for i=1,40 do local k: integer=i*7 occupied[k]=true "
            "if occupied[k] then total=total+i end end return total",
            820,
        ),
    ],
)
def test_sparse_storage_admits_three_unrelated_regions(program, expected):
    runtime = LuaRuntime(jit_threshold=1)
    source = "-- luapyre: typed\n" + program
    assert runtime.execute(source) == expected
    assert runtime.execute(source) == expected
    assert runtime.jit_stats.fast_path_admissions[
